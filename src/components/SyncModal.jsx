  ...
  const handleSyncNow = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        throw new Error(data.error || 'Local synchronization error.');
      }
    } catch (err) {
      // Fallback message if running static without local backend
      setError(err.message || 'Cannot reach local sync bridge. You can copy the generated configuration files or run the shell script below.');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">Local 1-Click Sync Bridge</h3>
              <p className="text-xs text-slate-400">Direct injection to ~/.cursorrules & ~/.claude/</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5 overflow-y-auto max-h-[75vh]">
          ...
          <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono font-semibold text-slate-400">Tools in your stack:</span>
              <span className="text-xs font-mono font-bold text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/30">
                {favoriteItems.length} selected
              </span>
            </div>
            <div className="text-xs text-slate-300 space-y-1 pt-1">
              <div className="flex items-center gap-2">
                <FolderCheck className="w-4 h-4 text-emerald-400" />
                <span>Cursor Destination: <code className="text-emerald-300 font-mono">~/.cursorrules</code></span>
              </div>
              <div className="flex items-center gap-2">
                <FolderCheck className="w-4 h-4 text-amber-400" />
                <span>Claude Destination: <code className="text-amber-300 font-mono">~/.claude/CLAUDE.md</code></span>
              </div>
              <div className="flex items-center gap-2 text-[11px] text-slate-500">
                <ShieldCheck className="w-3.5 h-3.5 text-cyan-400" />
                <span>Automatic safety backup (.bak) is generated before every write.</span>
              </div>
            </div>
          </div>
            ...
            >
              {loading ? (
                <>
                  ...
                  <span>Syncing to local files...</span>
                </>
              ) : (
                <>
                  <Zap className="w-5 h-5 fill-current" />
                  <span>⚡ Inject Stack to Local Machine (1-Click)</span>
                </>
              )}
            </button>
          </div>
          ...
          {result && (
            <div className="p-4 rounded-2xl bg-emerald-950/40 border border-emerald-500/40 text-xs text-emerald-200 space-y-2 animate-fadeIn">
              ...
              <p className="text-slate-300 text-[11px]">
                Your {result.itemsCount} rules and directives are active immediately in your Cursor IDE and Claude Code CLI.
              </p>
              <div className="font-mono text-[10px] text-emerald-400/90 bg-emerald-950/60 p-2 rounded-lg border border-emerald-800/40 space-y-1">
                <div>✓ Updated: {result.cursorPath}</div>
                <div>✓ Updated: {result.claudePath}</div>
              </div>
            </div>
          )}
          ...
          <div className="pt-2 border-t border-slate-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono font-medium text-slate-400 flex items-center gap-1.5">
                ...
                Terminal Script Alternative (CLI):
              </span>
              <button
