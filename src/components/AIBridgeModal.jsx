...
const AI_PLATFORMS = [
  {
    ...
    description: 'Direct connection with Google Antigravity 2.0 runtime, autonomous subagents, and native slash commands.',
    features: [
      'Multi-agent orchestration (/teamwork-preview)',
...
Example: ${item.example}`).join('\n\n')}`
  },
  {
    ...
    description: 'Dedicated bridge for Claude Code CLI, Claude 3.7 Sonnet thinking mode, and Claude Desktop with MCP.',
    features: [
...
- **Prompt Pattern**: ${item.example}`).join('\n\n')}`
  },
  {
    id: 'cursor',
    name: 'Cursor IDE',
...
# Example prompt: ${item.example}`).join('\n\n')}`
  },
  {
    ...
    description: 'Highly structured System Prompt generator for OpenAI Codex, GitHub Copilot CLI, and GPT-4o reasoning models.',
    features: [
  ...
  const handleLiveSync = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setSyncResult({ success: true, message: `Successfully connected and synced to ${currentPlat.name}!` });
        try { confetti({ particleCount: 50, spread: 70 }); } catch (_) {}
      } else {
        throw new Error(data.error || 'Synchronization error.');
      }
    } catch (err) {
      setSyncResult({ success: false, message: `Copy the configuration below into your ${currentPlat.configFile} file.` });
    } finally {
      setSyncing(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-bold text-white">AI Multi-Platform Gateway & Bridge</h3>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/30">
                  Zero-API Required
                </span>
              </div>
              <p className="text-xs text-slate-400">
                Instantly connect your tool stack to Claude, Antigravity, Codex & Cursor
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-200">
          ...
          <div className="p-4 rounded-2xl bg-slate-950/70 border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-slate-400">
                  Target file: <code className="text-amber-300">{currentPlat.configFile}</code>
                </span>
              </div>
            ...
            <button
              ...
              disabled={syncing}
              className="whitespace-nowrap px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-amber-500 hover:from-cyan-400 hover:to-amber-400 text-slate-950 font-bold text-xs shadow-md shadow-cyan-500/20 transition-all flex items-center gap-1.5 hover:scale-105"
            >
              {syncing ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4 fill-current" />}
              <span>⚡ Connect & Inject Stack</span>
            </button>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono font-bold text-cyan-400 flex items-center gap-1.5">
                ...
                Generated Configuration ({activeStack.length} tools active):
              </span>
              <button
