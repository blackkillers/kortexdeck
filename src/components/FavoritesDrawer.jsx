...
import { X, BookmarkCheck, Download, Trash2, ExternalLink, Zap } from 'lucide-react';
...
export default function FavoritesDrawer({ 
  ...
  const exportAsMarkdown = () => {
    let md = `# My Custom AI Stack & Commands (KortexDeck)\n\n`;
    md += `Exported on ${new Date().toLocaleDateString('en-US')} - Total: ${favoriteItems.length} tools\n`;
    md += `Powered by Cohen Web Studio (https://cohenwebstudio.com/kortexdeck)\n\n`;

    favoriteItems.forEach((item, idx) => {
      md += `### ${idx + 1}. ${item.title} (\`${item.command}\`)\n`;
      md += `- **Platform**: ${item.platform.toUpperCase()}\n`;
      md += `- **Category**: ${item.category}\n`;
      md += `- **Type**: ${item.type} | **Level**: ${item.difficulty}\n`;
      md += `- **Description**: ${item.description}\n`;
      md += `- **Syntax**:\n\`\`\`bash\n${item.syntax || item.command}\n\`\`\`\n`;
      md += `- **Example**: \`${item.example}\`\n\n---\n\n`;
    });
  ...
  const exportAsCursorRules = () => {
    let rules = `# .cursorrules - Generated by KortexDeck (Cohen Web Studio)\n\n`;
    rules += `# Custom stack of ${favoriteItems.length} curated commands & skills\n\n`;
    
    favoriteItems.forEach((item) => {
      rules += `## [${item.platform.toUpperCase()}] ${item.title}\n`;
      rules += `# Command: ${item.command}\n`;
      rules += `# Directive: ${item.description}\n`;
      if (item.arguments && item.arguments.length > 0) {
        rules += `# Arguments: ${item.arguments.map(a => a.name).join(', ')}\n`;
      }
      rules += `# Example: ${item.example}\n\n`;
    });
  ...
  const exportAsClaudeMD = () => {
    let claudemd = `# CLAUDE.md - Project Directives generated via KortexDeck\n\n`;
    claudemd += `> Auto-generated for Claude Code and Claude Desktop (${favoriteItems.length} active tools)\n\n`;
    claudemd += `## Directives & Recommended Tools\n\n`;

    favoriteItems.forEach((item, idx) => {
      claudemd += `### ${idx + 1}. ${item.title}\n`;
      claudemd += `- **Command**: \`${item.command}\`\n`;
      claudemd += `- **Use case**: ${item.summary}\n`;
      claudemd += `- **Syntax**: \`${item.syntax || item.command}\`\n\n`;
    });
        ...
        <div className="w-screen max-w-md bg-slate-900 border-l border-slate-800 shadow-2xl flex flex-col">
          ...
          <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-base font-bold text-white">My Custom AI Stack</h3>
                <p className="text-xs text-slate-400">{favoriteItems.length} commands saved</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {favoriteItems.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-500">
                ...
                <p className="text-sm font-medium text-slate-400">Your custom stack is empty</p>
                <p className="text-xs text-slate-600 mt-1 max-w-xs">
                  Click the bookmark icon on any command card to save it to your stack and sync it with 1-click.
                </p>
              </div>
            ) : (
          ...
          {favoriteItems.length > 0 && (
            <div className="p-4 border-t border-slate-800 bg-slate-950/80 space-y-2.5">
              ...
              >
                ...
                <span>⚡ Sync to ~/.cursorrules & ~/.claude/</span>
              </button>
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  title="Export as .cursorrules for Cursor IDE"
                >
                  .cursorrules
                </button>
                <button
                  ...
                  title="Export as CLAUDE.md for Claude Code"
                >
                  CLAUDE.md
                </button>
                <button
                  ...
                  title="Export as raw JSON format"
                >
                  JSON
                </button>
              </div>
